        ...
        let resultSelect = await db.sequelize.query(
            ...
            .then(resultSelect => {
                ...
                let resultToSend = resultSelect.map( item => {
                    return { id: item.CODIGO , descripcion : item.DESCRIP, precio: item.PRECIO, rubro: item.r_descrip }; 
...
controllers.login = async (req, res) => {
    ...
    if(mongo.usuarios === undefined) return res.status(401).json("Datos Incorrectos");
...
controllers.misPedidosDetalle = async (req, res) => {
    ...
    let productos = await db.sequelize.query(
